...
public interface HospitalRepository  extends JpaRepository<Hospital, Integer> {
    @Query(value = "SELECT distinct * FROM hospital h inner join speciality_hospitals sh on sh.hospitals_id=h.id and sh.specialities_id=:specialityId WHERE (h.latitude between :latitudeRight and :latitudeLeft) AND (h.longitude between :longitudeRight and :longitudeLeft) AND (h.available_beds > 0) LIMIT 10", nativeQuery = true)
