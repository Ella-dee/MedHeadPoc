...
public interface HospitalRepository  extends JpaRepository<Hospital, Integer> {
    @Query(value = "SELECT distinct * FROM hospital h left join speciality_hospitals sh on  hospitals_id=h.id and sh.hospitals_id=:specialityId WHERE (h.latitude between :latitudeRight and :latitudeLeft) AND (h.longitude between :longitudeRight and :longitudeLeft) AND (h.available_beds > 0) LIMIT 5", nativeQuery = true)
